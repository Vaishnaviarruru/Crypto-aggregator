...
const backoff_1 = __importDefault(require("../utils/backoff"));
async function fetchFromDexScreener(q) {
    try {
        console.log(`🔍 Fetching from DexScreener for query: ${q}`);
        const res = await backoff_1.default.get(`https://api.dexscreener.com/latest/dex/search?q=${q}`);
        if (!res.data || !res.data.pairs) {
            console.error("❌ DexScreener returned no pairs");
            return [];
        }
        // Map results to your Token type
        const tokens = res.data.pairs.map((p) => ({
